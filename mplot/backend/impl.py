...
from matplotlib.backend_bases import (
    _Backend, Event, KeyEvent, LocationEvent, MouseEvent, ResizeEvent)
...
class FigureCanvasWebAggCore(backend_agg.FigureCanvasAgg):
    ...
    # we can debug this.
    supports_blit: bool = False # pyright: ignore[reportIncompatibleVariableOverride]

    def __init__(self, figure: Figure):
        super().__init__(figure=figure)
        # Set to True when the renderer contains data that is newer
        # than the PNG buffer.
        self._png_is_old: bool = True
        # Set to True by the `refresh` message so that the next frame
        # sent to the clients will be a full frame.
        self._force_full: bool = True
        # The last buffer, for diff mode.
        self._last_buff: npt.NDArray[np.uint32] = np.empty((0, 0), dtype=np.uint32)
        # Store the current image mode so that at any point, clients can
        # request the information. This should be changed by calling
        # self.set_image_mode(mode) so that the notification can be given
        # to the connected clients.
        self._current_image_mode: Literal["full", "diff"] = 'full'
        # Track mouse events to fill in the x, y position of key events.
        self._last_mouse_xy: tuple[float | None, float | None] = (None, None)
    ...
    def draw(self) -> None:
        ...
        try:
            ...
            if isinstance(self.manager, FigureManagerWebAgg):
    ...
    def draw_idle(self, *args: Any, **kwargs: Any) -> None:
    ...
    def set_image_mode(self, mode: Literal["full", "diff"]) -> None:
        """
...
class NavigationToolbar2WebAgg(backend_bases.NavigationToolbar2):
    ...
    def set_message(self, s: str) -> None:
        if s != self.message:
            self.canvas.send_event("message", message=s)
        self.message = s
